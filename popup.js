    ...
    try {
        ...
        if (result.is_relevant === false) {
            document.getElementById("result").innerHTML =
                ...
                `<span style="color: red; font-size: 16px;"><strong>Unrelated Query:</strong> ${result.message || "The input is unrelated to diabetes."}</span>`;
...
function displayExplainability(text, keyWords, explanation) {
    ...
    if (!keyWords || keyWords.length === 0) {
        ...
        // Tokenize the sentence into words, then wrap matches
        const words = text.split(/\b/);  // split into word boundaries
        const lowerKeywords = keyWords.map(k => k.toLowerCase());

        const highlightedWords = words.map(word => {
            if (lowerKeywords.includes(word.toLowerCase())) {
        ...
        const finalText = highlightedWords.join("");
        highlightedTextContainer.innerHTML = `<strong>Highlighted Text:</strong> ${finalText}`;
    }
